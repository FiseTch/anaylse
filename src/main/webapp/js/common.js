...
function zy_Countdown() {
	...
	0 >= zy_c_num && (zy_c_num = 60, $(".sendE2").hide(), $(".sendE").show(), clearInterval(sendECount))
}
function login_form_submit() {    
	...
	/*$.ajax({
		...
		success: function (data) {
		    alert(data);
		    if(data != null){
		    	window.location.href="../../../anaylse/view /user/downExcel.jsp?user"+data;
		    }else{
			window.location.href="../../../anaylse/view/result/uploadFailure.jsp"
		    }
		   
		    $("#login_btn").html("登录成功");
		    clearBox();//清空输入框
		    $("#login-form-tips").html("正在跳转").show(3000);//消息显示框提留3秒
			...
			if (e.account_l)
				$("#login-form-tips").html(e.account_l).show(1000), $("#id_account_l").focus();//消息显示框提留1秒
			else if (e.password_l)
				$("#login-form-tips").html(e.password_l).show(1000), $("#id_password_l").focus();//消息显示框提留1秒
			else {
				if ("success" == e.status){
				    clearBox();//清空输入框
				    return "True" == e.onepay_status ? window.location.replace("/anaylse/") : window.location.replace(e.url), detect_ie() !== !1 && window.location.reload(), void 0;
				    "failure" == e.status && ("no_active" == e.msg ? (zyemail = $("#id_account_l").val(), zyUname = zyemail, zy_validate_Email(!1, zyemail, $("#id_password_l").val())) : $("#login-form-tips").html("账号或者密码错误，请重新输入").show(1000))
				}
			}
		},		
		complete: function () {//不管是否ajax是否调用成功，都执行
			$("#login_btn").removeAttr("disabled");//登录成功就移除标识                                          
		},
		error: function (data) {
...
function show_send_sms(e) {
	...
	0 >= e ? ($(".verify-tips.success").hide(500), $("#forgetpswMobileModal .send-verify,#verify_form .send-verify").html("重发验证码").css({
			background: "#5ECFBA"
...
function send_sms_code() {
	...
	s = "";
	("" == e || void 0 == e) && (e = $("#id_mobile_f").val(), s = "change"),
	sendOFF && ($.ajax({
			cache: !1,
			type: "POST",
...
function cutTimer(e, s) {
	window.setInterval(function () {
		...
		t = 0,
		i = 0;
		e > 0 && (o = Math.floor(e / 86400), a = Math.floor(e / 3600) - 24 * o, t = Math.floor(e / 60) - 60 * 24 * o - 60 * a, i = Math.floor(e) - 60 * 60 * 24 * o - 60 * 60 * a - 60 * t),
		9 >= t && (t = "0" + t),
		9 >= i && (i = "0" + i),
		$(s + "#day_show").html(o + "天"),
...
function imgPopup(e) {
	...
	o.onload = function () {
		...
		s = o.width,
		e = o.height,
		a.width() / a.height() >= s / e && (e > a.height() ? (a.find(".img").width(parseInt(a.height() * s / e)), a.find(".img").height(parseInt(a.height())), a.width(parseInt(a.height() * s / e)), a.height(parseInt(a.height()))) : (a.width(s), a.height(e))),
		a.css({
			left: "50%",
			top: "50%",
...
function Cuttime(e, s) {
	...
	o = setInterval(function () {
			s--,
			e.val(s + "s"),
			0 >= s && (clearInterval(o), e.removeClass("send").val("重新发送").removeAttr("disabled"))
		}, 1e3)
}
...
Search(), $(document).ready(function () {
	...
	$("#data-search").keyup(function () {
		...
		"" == keywd ? (listbox_hide(), flayer_show(), void 0) : (searchajax(keywd), void 0)
	}).blur(function () {
		return keywd = $("#data-search").val(),
		"" == keywd ? (listbox_hide(), void 0) : (listbox_hide(500), void 0)
	})
});
